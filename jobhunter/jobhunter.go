...
func textReadFromScreen(x_min, y_min, x_max, y_max int) string {
	...
	cropedScreenshotFilePath := screenshotCrop(
		screenshotFilePath, x_min, y_min, x_max, y_max)

	textFilePath := nextFileNameInStorage()
	exec.Command(
		"tesseract", cropedScreenshotFilePath, textFilePath, "--psm", "7").Run()
...
func textWaitWhileShowsAndmouseMoveAndLeftClick(
	x_min, y_min, x_max, y_max int, t string) bool {
...
func screenshotCrop(
	screenshotFilePath string, x_min, y_min, x_max, y_max int) string {
...
func browserTypeAndEnterURLToURLBar(l string) {
	mouseMoveAndLeftClick(800, 95)
	textTypeAndEnter(l)
}

func browserScrollDownHalfPage(x int, y int) {
	mouseMoveAndLeftClick(x, y)
	scrollDown(15)
}
...
// START OF SITE SPECIFIC FUNCS

// END OF SITE SPECIFIC FUNCS
